...
# ==============================================================================
# CORREÇÃO: Gerar Keystore antecipadamente onde o Gradle exige
# ==============================================================================
log_info "Pre-generating keystore for Gradle..."
mkdir -p android
...
# ==============================================================================
# 7. Prepare Structure for androiddeployqt (Injeção do .so e da Keystore)
# ==============================================================================
log_info "Preparing folder structure and injecting assets..."

# Cria implicitamente a árvore esperada pela ferramenta de deployment
mkdir -p android-build/libs/arm64-v8a

# INJEÇÃO DA KEYSTORE: Resolve o erro ':validateSigningRelease'
cp "../$KEYSTORE_NAME" android-build/debug.keystore
log_info "Keystore injected into android-build/debug.keystore"
...
if [ -n "$SO_ORIGINAL" ] && [ -f "$SO_ORIGINAL" ]; then
    ...
    log_error "Native binary (lib2x2coin-wallet.so) was not generated by 'make'."
fi
